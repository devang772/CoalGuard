...
function KpiCard({ label, value, delta, deltaLabel, icon, urgent }: { label: string; value: unknown; delta?: number; deltaLabel?: string; icon: React.ReactNode; urgent?: boolean }) {
  return (
  ...
  return (
    <div className="dashboard-card p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold">Recent attendance (today)</h3>
                  ...
                  return (
                    <div key={fmt(m.mine_id ?? m.id)} className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer hover:bg-muted/40 ${level === "high" ? "border-red-200 bg-red-50/30" : ""}`} onClick={() => onNavigate("Mines")}>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{fmt(m.top_reason ?? reasons[0]?.factor)}</p>
                      </div>
                      <div className="flex items-center gap-2">
